...
class NewApplicationForm(forms.ModelForm):
    id_commission_type = forms.ModelChoiceField(queryset=CommissionType.objects.all(),empty_label="Seleccione Tipo de Comisión")#
...
class DestinationForm(forms.ModelForm):
    ...
    start_date = forms.DateField(input_formats=['%d/%m/%y'],widget=forms.DateInput(attrs={'class':'datepicker', 'data-date-format':"dd/mm/yy"}))#
    end_date = forms.DateField(input_formats=['%d/%m/%y'],widget=forms.DateInput(attrs={'class':'datepicker', 'data-date-format':"dd/mm/yy"}))#widget=SelectDateWidget()
    class Meta:
        model = Destination
        exclude = {'application'}
