...
# for Historic Calendar
class HistoricCalendar(HTMLCalendar):
    def __init__(self, workouts):
        super(HistoricCalendar, self).__init__()
        self.workouts = self.group_by_day(workouts)
    ...
    def formatday(self, day, weekday):
        if day != 0:
            ...
            if day in self.workouts:
                ...
                body = ['<ul>']
                for dest in self.workouts[day]:
                    body.append('<a href="application_review?id=%s">' % dest.application_id)
                    body.append(esc(dest.application.id_Teacher))
                    body.append('<br>')
                    body.append(esc(dest.city))
                    body.append(', ')
                    body.append(esc(dest.country))
    ...
    def formatmonth(self, year, month):
        self.year, self.month = year, month
        return super(HistoricCalendar, self).formatmonth(year, month)
